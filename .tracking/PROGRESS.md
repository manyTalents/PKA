...
**Monday fixes — ✅ ALL DEPLOYED LIVE 2026-06-28** (Owner "a — do everything we can"): invoice branding + constants → backend (Forge, merge 6ae774f, backup 20260628_155426, migrate clean, print format renders 10/10 vs ACC-SINV-2026-00003, ping=pong); web office-parity → Vercel (Glass, merge cf05aaa+ab0eb87, deploy success). Pending Owner inputs for full branding: logo (ERPNext Letter Head) + google_review_url. **Offline re-test (QUE-02/03) BLOCKED + DEFERRED** — emulator DNS wedged (pings 8.8.8.8 but resolves nothing, both wifi+cellular; Play image = no root; needs host reboot/fresh AVD). Built details:
...
## 2026-06-28 — Watchtower congress-trades + TA layer: brainstorm → spec → plan → Phase A BUILT
Session continued from Phase-1 deploy. Owner asked re: options/TA off Watchtower → brainstormed a TA confirmation layer, found data sources in BW (Polygon free tier verified for daily OHLCV; Alpaca + Tradier also present), folded TA + the green-lit congress-trades domain into ONE build (shared confluence/schema/scoring).
- **Design spec** `.tracking/specs/2026-06-28-watchtower-ta-layer-design.md` + **plan** `2026-06-28-watchtower-congress-ta.md` (committed eb3f913, 75ae2c5).
- **Phase A COMPLETE** via subagent-driven development (Kit implementer + per-task spec/quality review + fix loops; Rex quant-reviewed the alpha math). Branch `feat/congress-trades-ta`, 8 commits, **86 tests green**, NOT deployed:
  - A1 schema (political_trades/member_performance/ta_snapshots) `b6bfc4b`
  - A2 House Clerk PTR fetch+parse — built against REAL live format (learned actual XML+PDF structure first per the Phase-0.5 lesson), pdfplumber already vendored `203351e` + fix `5636295` (single-letter-ticker false-neg + injectable sleep)
  - A3 dedup store `e0afee8`
  - A4 disclosure-anchored dollar-weighted alpha ranking + watchlist + Yahoo historical price helpers `9600ee6` + fix `a89f499` (anchor-discriminating test + utcfromtimestamp). Rex verified DW math correct.
  - A5 threshold + cluster alerts + run() (real send_alert 4-arg sig, inline price-at-detection, 45-day-lag footer) `a2021f7`
  - A6 confluence ticker-overlap domain + scheduler (political_trades_daily 06:30, member_alpha_monthly) + config + day-1 seed + cluster recency gate `9207435`
- **Remaining:** Phase B = TA confirmation layer (B1 Polygon→Yahoo fetch, B2 indicators+verdict, B3 confluence confirm/veto + ta_snapshots, B4 forward-return accuracy); Phase C = deploy (RED, add POLYGON_API_KEY to droplet .env). Minor findings parked for the final whole-branch review (docstrings, a couple test-precision items).
